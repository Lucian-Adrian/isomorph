...
Functional Definition: A Device is strictly defined as peripheral equipment that DOES NOT PROCESS data (NU PRELUCREAZĂ date). In the current DSL this is represented as a deployment element rendered through the shared component/deployment renderer. Device-vs-node hosting rules should be treated as product constraints only when backed by analyzer tests; do not assume an unimplemented validator warning exists.
