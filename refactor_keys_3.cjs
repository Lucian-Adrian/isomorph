...
const filesToProcess = getFiles('src').concat(getFiles('website'));
...
filesToProcess.forEach(file => {
  ...
  for (let i = 0; i < sortedKeys.length; i++) {
    const enStr = sortedKeys[i];
    const rKey = mappings[enStr];
    
    // Easy constant string replacements
    let toReplace = "t('" + enStr + "')";
    if (content.indexOf(toReplace) !== -1) { content = content.split(toReplace).join("t('" + rKey + "')"); changed = true; }
    
    toReplace = 't("' + enStr + '")';
    if (content.indexOf(toReplace) !== -1) { content = content.split(toReplace).join("t('" + rKey + "')"); changed = true; }
    
    // For when parameter is passed
    toReplace = "t('" + enStr + "', {";
    if (content.indexOf(toReplace) !== -1) { content = content.split(toReplace).join("t('" + rKey + "', {"); changed = true; }
    ...
    toReplace = "'" + enStr + "':";
    if (content.indexOf(toReplace) !== -1) { content = content.split(toReplace).join("'" + rKey + "':"); changed = true; }
