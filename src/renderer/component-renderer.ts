...
function renderComponent(p: Placed): string {
  ...
  s += `    <rect width="${BOX_W}" height="${COMP_H}" rx="6" fill="#f0f9ff" stroke="#3b82f6" stroke-width="1.5" filter="url(#shadow)"/>\n`;
  ...
  s += `    <text x="${BOX_W / 2}" y="33" text-anchor="middle" font-size="13" font-weight="600" fill="#0f172a">${escapeXml(entity.name)}</text>\n`;
  // Draw ports
  ...
  provided.forEach((f, i) => {
    ...
    s += `    <circle cx="${BOX_W + 20}" cy="${py}" r="5" fill="white" stroke="#3b82f6" stroke-width="1.5"/>\n`;
    s += `    <text x="${BOX_W + 28}" y="${py + 3}" font-size="10" fill="#334155">${escapeXml(f.name)}</text>\n`;
  });
  ...
  required.forEach((f, i) => {
    ...
    s += `    <text x="-24" y="${py + 3}" text-anchor="end" font-size="10" fill="#334155">${escapeXml(f.name)}</text>\n`;
  });
  
  ports.forEach((f, i) => {
    const px = 20 + i * 20;
    s += `    <rect x="${px - 4}" y="${COMP_H - 4}" width="8" height="8" fill="white" stroke="#3b82f6" stroke-width="1.5"/>\n`;
    s += `    <text x="${px}" y="${COMP_H + 14}" text-anchor="middle" font-size="10" fill="#334155">${escapeXml(f.name)}</text>\n`;
  });
...
function renderArtifact(p: Placed): string {
  ...
  let s = `  <g transform="translate(${x},${y})" data-entity-name="${escapeXml(entity.name)}">\n`;
  s += `    <path d="M 0 0 L ${BOX_W - 12} 0 L ${BOX_W} 12 L ${BOX_W} ${COMP_H} L 0 ${COMP_H} Z" fill="#fdf4ff" stroke="#a855f7" stroke-width="1.5" filter="url(#shadow)"/>\n`;
  ...
  s += `    <text x="${BOX_W / 2}" y="36" text-anchor="middle" font-size="13" font-weight="600" fill="#0f172a">${escapeXml(entity.name)}</text>\n`;
  s += `  </g>\n`;
  return s;
}
...
function renderNode(p: Placed): string {
  ...
  let s = `  <g transform="translate(${x},${y})" data-entity-name="${escapeXml(entity.name)}">\n`;
  // 3-D box top face
  s += `    <polygon points="0,${d} ${d},0 ${w + d},0 ${w},${d}" fill="#dcfce7" stroke="#22c55e" stroke-width="1.5"/>\n`;
  // Right face
  s += `    <polygon points="${w},${d} ${w + d},0 ${w + d},${h} ${w},${h + d}" fill="#bbf7d0" stroke="#22c55e" stroke-width="1.5"/>\n`;
  // Front face
  ...
  s += `    <text x="${w / 2}" y="${d + 35}" text-anchor="middle" font-size="13" font-weight="600" fill="#0f172a">${escapeXml(entity.name)}</text>\n`;
  s += `  </g>\n`;
  return s;
}
