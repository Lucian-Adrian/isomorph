...
function renderEntity(p: Placed): string {
  ...
  if (entity.kind === 'actor') {
     ...
     const cx = BOX_W / 2;
     s += `    <circle cx="${cx}" cy="15" r="10" fill="#f8fafc" stroke="#475569" stroke-width="1.5" filter="url(#shadow)"/>\n`;
     s += `    <line x1="${cx}" y1="25" x2="${cx}" y2="50" stroke="#475569" stroke-width="1.5"/>\n`;
     s += `    <line x1="${cx - 15}" y1="35" x2="${cx + 15}" y2="35" stroke="#475569" stroke-width="1.5"/>\n`;
     s += `    <line x1="${cx}" y1="50" x2="${cx - 10}" y2="70" stroke="#475569" stroke-width="1.5"/>\n`;
     s += `    <line x1="${cx}" y1="50" x2="${cx + 10}" y2="70" stroke="#475569" stroke-width="1.5"/>\n`;
     s += `    <text x="${cx}" y="85" text-anchor="middle" font-size="12" font-weight="500" fill="#0f172a">${escapeXml(nameOnly)}</text>\n`;
  } else {
    // Object, Active Object, Multiobject
    const textHtml = `<text x="${BOX_W / 2}" y="30" text-anchor="middle" font-size="13" font-weight="600" text-decoration="underline" fill="#0f172a">${escapeXml(label)}</text>`;
    
    if (entity.kind === 'multiobject') {
      s += `    <rect x="5" y="-5" width="${BOX_W}" height="${BOX_H}" fill="#ffffff" stroke="#64748b" stroke-width="1.5"/>\n`;
      s += `    <rect x="0" y="0" width="${BOX_W}" height="${BOX_H}" fill="#f8fafc" stroke="#64748b" stroke-width="1.5" filter="url(#shadow)"/>\n`;
    } else if (entity.kind === 'active_object') {
      s += `    <rect width="${BOX_W}" height="${BOX_H}" fill="#f8fafc" stroke="#64748b" stroke-width="2" filter="url(#shadow)"/>\n`;
      s += `    <line x1="10" y1="0" x2="10" y2="${BOX_H}" stroke="#64748b" stroke-width="1"/>\n`;
      s += `    <line x1="${BOX_W - 10}" y1="0" x2="${BOX_W - 10}" y2="${BOX_H}" stroke="#64748b" stroke-width="1"/>\n`;
    } else {
      // standard object or composite object
      s += `    <rect width="${BOX_W}" height="${BOX_H}" fill="#f8fafc" stroke="#64748b" stroke-width="1.5" filter="url(#shadow)"/>\n`;
    }
    s += textHtml + '\n';
  }
