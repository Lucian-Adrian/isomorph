...
export function renderUseCaseDiagram(diag: IOMDiagram): string {
  ...
  for (const { e, p } of ucPositions) {
    ...
    svg += `    <ellipse cx="0" cy="0" rx="${UC_RX}" ry="${ry}" fill="#eff6ff" stroke="#3b82f6" stroke-width="1.5" filter="url(#shadow)"/>`;
