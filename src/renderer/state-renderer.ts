...
function renderActivitySwimlanes(partitions: IOMEntity[], placed: Placed[], height: number): string {
  ...
  for (let i = 0; i < partitionPlacements.length; i++) {
    ...
    laneSvg += `<rect data-lane-body="true" x="0" y="0" width="${laneW}" height="${laneH}" rx="8" fill="var(--iso-bg-panel)" stroke="#cbd5e1" stroke-width="1"/>`;
    laneSvg += `<rect data-lane-header="true" x="0" y="0" width="${laneW}" height="${headerHeight}" rx="8" fill="var(--iso-divider)" stroke="#cbd5e1" stroke-width="1"/>`;
    laneSvg += `<line data-lane-divider="true" x1="0" y1="${headerHeight}" x2="${laneW}" y2="${headerHeight}" stroke="#cbd5e1" stroke-width="1"/>`;
    laneSvg += `<text data-lane-title="true" x="${laneW / 2}" y="22" text-anchor="middle" font-size="12" font-weight="600" fill="var(--iso-text-body)">${escapeXml(lane.partition.name)}</text>`;
...
function renderEntity(p: Placed): string {
  ...
  if (entity.kind === 'start') {
    ...
    s += `    <rect width="${BAR_W}" height="${BAR_H}" rx="2" fill="var(--iso-text-body)" filter="url(#shadow)"/>\n`;
    ...
    if (intActs.length > 0) {
      ...
      for (const act of intActs) {
        s += `    <text x="10" y="${my}" font-size="11" fill="var(--iso-text-body)"><tspan font-weight="600">${escapeXml(act.name)}</tspan> / ${escapeXml(act.returnType)}</text>\n`;
        my += 15;
      }
    }
  }
